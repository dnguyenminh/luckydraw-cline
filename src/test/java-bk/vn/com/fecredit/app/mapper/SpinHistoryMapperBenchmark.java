...
public class SpinHistoryMapperBenchmark {
    ...
    public static class BenchmarkState {
        ...
        SpinResponse.CreateRequest testRequest;
        List<SpinHistory> entityList;
        List<SpinResponse.CreateRequest> requestList;
        ...
        private List<SpinResponse.CreateRequest> createRequestList() {
            return IntStream.range(0, LIST_SIZE)
    ...
    public void toResponse(BenchmarkState state, Blackhole blackhole) {
        SpinResponse.SpinResponse response = state.mapper.toResponse(state.testEntity);
    ...
    public void toResponseList(BenchmarkState state, Blackhole blackhole) {
        List<SpinResponse.SpinResponse> responses = state.mapper.toResponseList(state.entityList);
    ...
    public void parallelToResponseList(BenchmarkState state, Blackhole blackhole) {
        List<SpinResponse.SpinResponse> responses = state.entityList.parallelStream()
    ...
    public void calculateStatistics(BenchmarkState state, Blackhole blackhole) {
        SpinResponse.Statistics stats = state.mapper.calculateStatistics(state.entityList);
        blackhole.consume(stats);
    }

    public static void main(String[] args) throws Exception {
        Options opt = new OptionsBuilder()
            .include(SpinHistoryMapperBenchmark.class.getSimpleName())
            .forks(1)
            .warmupIterations(3)
