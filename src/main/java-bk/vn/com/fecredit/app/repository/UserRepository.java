...
public interface UserRepository extends JpaRepository<User, Long>, JpaSpecificationExecutor<User> {
    ...
    List<User> findAllByStatus(@Param("status") EntityStatus status);

    @Query("SELECT u FROM User u WHERE u.status = :status")
    Page<User> findAllByStatus(@Param("status") EntityStatus status, Pageable pageable);
    ...
    void updateStatus(@Param("id") Long id, @Param("status") EntityStatus status);
    ...
    @Query("SELECT u FROM User u WHERE u.status = 1 AND u.status = 1")
    List<User> findAllstatusAndActive();

    @Query("SELECT COUNT(u) FROM User u WHERE u.status = 1 AND u.status = 1")
    long countstatusAndActive();
}
